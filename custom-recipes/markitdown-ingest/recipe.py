...
if folder_handle:
    ...
    for path in paths:
        with folder_handle.get_download_stream(path) as file_to_convert:
            ...
            converted_data = None
            try:
                converted_data = markitdown.convert(temporary_location.name + path)
            except Exception as error_message:
                output_row['error'] = "{}".format(error_message)
            output_row['file_path'] = path
            if converted_data:
                output_row['markdown_document'] = converted_data.text_content
            else:
                output_row['markdown_document'] = None
    ...
    for index, input_row in documents_to_process_dataframe.iterrows():
        ...
        converted_data = None
        try:
            converted_data = markitdown.convert(url)
        except Exception as error_message:
                output_row['error'] = "{}".format(error_message)
        if converted_data:
            output_row['markdown_document'] = converted_data.text_content
